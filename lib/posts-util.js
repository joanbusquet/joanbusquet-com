...
export function getAllPosts() {
  ...
  // Filter to only show today or past posts
  const today = new Date()
  const allPostsFiltered = allPosts.filter(
    (post) => new Date(post.date) < today
  )

  // Sort all posts by post date
  const sortedPosts = allPostsFiltered.sort((postA, postB) =>
